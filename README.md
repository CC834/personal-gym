...
- Configuration: `$HOME/.config/personal-gym/config.json`
- SQLite data: `$HOME/.local/share/personal-gym/gym.sqlite3`
- Licensed media: `$HOME/.local/share/personal-gym/catalog-media/`
...
```sh
GYM_CONFIG="$HOME/.config/personal-gym/config.json" npm run catalog:import -- --source /path/to/exercises.json --revision <commit-sha>
```
...
```sh
GYM_CONFIG="$HOME/.config/personal-gym/config.json" npm run catalog:media -- --source /path/to/licensed/exercises-dataset
```
